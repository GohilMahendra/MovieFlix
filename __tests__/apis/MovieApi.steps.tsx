...
import {
  apiResponseMovie,
  apiResponseNowPlaying,
  apiResponseNowPlaying2,
  apiSerachResultResponse,
} from '../mocks/MockData';
import {
  fetchMovieDetails,
  fetchMovies,
  searchMovies,
} from '../../src/apis/MovieApi';
describe('Movie Api unit tests', () => {
  ...
  it('It can fetch movies for page 1 successfully', async () => {
    mockAxios
      .onGet(`${BASE_URL}movie/now_playing?language=en-US&page=1`)
      .reply(200, apiResponseNowPlaying);
  ...
  it('It will throw error if any issue found', async () => {
    ...
    mockAxios
      .onGet(`${BASE_URL}movie/${category}?language=en-US&page=1`)
      .reply(500, {error: errorMessage});
    await expect(fetchMovies('now_playing')).rejects.toThrow(
  ...
  it('It can can fetch more now playing for page 2 also', async () => {
    mockAxios
      .onGet(`${BASE_URL}movie/now_playing?language=en-US&page=2`)
      .reply(200, apiResponseNowPlaying2);
  ...
  it('It can can fetch the search result from api', async () => {
    mockAxios
      .onGet(
        `${BASE_URL}search/movie?query=die&include_adult=false&language=en-US&page=1`,
      )
      .reply(200, apiSerachResultResponse);
  ...
  it('It can can throw error if anything happens with sevrer', async () => {
    ...
    mockAxios
      ...
      .reply(500, {error: errorMessage});
    await expect(searchMovies('die')).rejects.toThrow(
      'Error: Request failed with status code 500',
    );
  });
  ...
  it('I can fetch the movie details with help of movie id', async () => {
    ...
    mockAxios
      .onGet(`${BASE_URL}movie/${movieId}?language=en-US`)
      .reply(200, apiResponseMovie);
